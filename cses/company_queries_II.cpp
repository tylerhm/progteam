...
int main() {
	...
	for (int i = 1; i < n; i++) {
		...
		adj[i].emplace_back(boss, 1);
		adj[boss].emplace_back(i, 1);
	}

	TreeQueries<int, function<int(int, int)>> t(
		adj, [](const int a, const int b){return 0;}, 0);

	for (int i = 0; i < q; i++) {
		int a, b; cin >> a >> b; a--; b--;
		cout << t.query(a, b).first + 1 << nl;
	}
